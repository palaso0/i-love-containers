...
export const ComposeDiagramCanvas: React.FC<ComposeDiagramCanvasProps> = ({
  ...
  useEffect(() => {
    if (initializedProjectRef.current !== projectName && containerRef.current) {
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        updateView({ x: nextX, y: nextY, zoom: nextZoom });
      } else {
  ...
  useEffect(() => {
    const handleGlobalMouseMove = (e: MouseEvent) => {
      ...
      if (draggingNodeRef.current) {
        const dx = (e.clientX - draggingNodeRef.current.startX) / viewRef.current.zoom;
        const dy = (e.clientY - draggingNodeRef.current.startY) / viewRef.current.zoom;

        if (!draggingNodeRef.current.hasMoved && Math.hypot(dx, dy) > 3) {
          draggingNodeRef.current.hasMoved = true;
        }

        if (draggingNodeRef.current.hasMoved) {
          const nextX = Math.round(draggingNodeRef.current.nodeStartX + dx);
          const nextY = Math.round(draggingNodeRef.current.nodeStartY + dy);
          nodePositionsRef.current = {
            ...nodePositionsRef.current,
