...
import { WorkspaceSession, useWorkspaceStore } from "@/stores/useWorkspaceStore";
...
export const UnifiedLogsViewer: React.FC<UnifiedLogsViewerProps> = ({ session, isActive }) => {
      ...
      try {
        if (session.composeProject) {
          ...
          if (isMounted && combined.length > 0) {
            combined.slice(-100).forEach((entry) => appendLog(session.id, entry));
          }
  ...
  }, [session.id, session.containerId, session.containerName, session.composeProject, isPaused, isActive, isDark, appendLog]);
